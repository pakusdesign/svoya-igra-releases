...
export function GameBoard({ state, setState }: Props) {
  ...
  return (
    <main className="min-h-screen bg-board p-5 text-ink">
      ...
      <section className="overflow-x-auto rounded-lg border border-boardLine">
        <div className="min-w-[900px]">
          {round.themes.map((theme) => (
            <div className="grid grid-cols-[260px_repeat(5,1fr)] border-b border-boardLine last:border-b-0" key={theme.id}>
              <div className="flex items-center bg-[#0d2a66] p-4 text-2xl font-black">{theme.title}</div>
              {DEFAULT_PRICES.map((price) => {
                const question = theme.questions.find((item) => item.price === price);
                const cellClass = `grid min-h-28 place-items-center border-l border-boardLine bg-board p-4 text-5xl font-black text-prize transition-colors ${
